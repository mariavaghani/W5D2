...
def ford_supporting_films
  ...
  JOIN castings ON movies.id = castings.movie_id
  WHERE
    castings.ord != 1 AND castings.actor_id = (
      SELECT
        actors.id
      FROM
        actors
...
def andrews_films_and_leads
  ...
  execute(<<-SQL)
  SELECT
    movies_with_andrews.title, actors.name
  FROM
    (
      SELECT
        *
    ...
    ) AS movies_with_andrews
  JOIN castings ON movies_with_andrews.id = castings.movie_id
...
def colleagues_of_garfunkel
  ...
  JOIN castings ON movies_with_garfunkel.id = castings.movie_id
  JOIN actors ON castings.actor_id = actors.id
  WHERE
        actors.name != 'Art Garfunkel'
